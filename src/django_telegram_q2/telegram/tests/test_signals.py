...
class SignalOrchestrationTests(TestCase):
    ...
    @patch(
        "django_telegram_q2.telegram.signals.transaction.on_commit",
        side_effect=lambda fn: fn(),
    )
    @patch("django_telegram_q2.telegram.signals.async_task")
    def test_created_schedules_worker(self, mock_async_task, mock_on_commit):
        ...
        mock_async_task.assert_any_call(
            "django_telegram_q2.telegram.tasks.telegram_ack", job.pk
        )
    ...
    def test_processing_finished_schedules_delivery(
        ...
        mock_async_task.assert_called_once_with(
            "django_telegram_q2.telegram.tasks.telegram_deliver", job.pk
        )
