...
History follows Conventional Commit-style subjects such as `feat:`, `build:`, `ci:`, and scoped forms like `build(deps):`. Keep subjects imperative and focused, and create commits with `git commit --signoff` because the repository enforces DCO sign-off. PRs should explain the behavior change, note configuration or migration impacts, link relevant issues, and include test results. Add screenshots only for admin or other visible UI changes; update `README.md` and `CHANGELOG.md` when public behavior changes.
